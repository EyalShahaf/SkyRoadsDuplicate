...
### 🎮 Gameplay
- **Hand-crafted, data-driven levels** — Levels 1-6 defined in external JSON files for easy modding and balance tweaks
- **Dynamic difficulty** — forward speed, hazard probability, and level-specific style ramps, capped at a configurable ceiling
...
- **Enhanced visual scene dressing** — stars at varying distances, comets with tails, distant nebulae/galaxies, mountain silhouettes, floating decorative cubes with glow halos, ambient particles, Tron-style grid, scrolling track bands, and speed streaks
...
- **Comprehensive logging** — runtime events, performance metrics, and asset loading tracked in `skyroads.log`
- **Crash reporting** — captures stack traces and system state in `crash.log` for easier debugging
...
| **O** | Take screenshot |
| **Esc** | Pause / Back to menu / Exit (with confirmation) |
| **P** | Pause |
...
│   ├── Log.hpp / .cpp      #   File and console logging system
│   ├── CrashHandler.hpp/.cpp#  Signal handling and crash log generation
...
├── assets/
│   ├── levels/             #   Level definitions (JSON) — geometry, style, and hazards
