...
enum class FinishStyle : int {
    ...
    PrecisionCorridor = 3, // Level 3: Tighter precision corridor + animated chevrons
...
struct Level {
    ...
    FinishZone finish{};        // Finish line zone (zero-initialized = no finish for placeholders)
    StartZone start{};           // Start line zone (zero-initialized = no start for placeholders)
};
...
// Check if player has crossed the finish zone. Returns true when player Z passes finish.endZ.
bool CheckFinishZoneCrossing(const Level& level, float playerZ);

// Get player spawn position from start zone. Returns spawnZ if start zone exists, otherwise default.
float GetSpawnZ(const Level& level);

// Assign deterministic visual variants to segments and obstacles based on their properties.
// This ensures all players see the same visual variety (deterministic, not random).
