...
int main() {
    ...
    SetExitKey(0);  // Disable raylib's default ESC=quit so we handle ESC ourselves.
    ...
    while (!WindowShouldClose() && !game.wantsExit) {
        ...
        if (game.screen == GameScreen::Playing) {
            ...
            while (game.accumulator >= cfg::kFixedDt && simSteps < kMaxSimStepsPerFrame) {
        ...
        game.updateMs = std::chrono::duration<float, std::milli>(updateEnd - updateStart).count();
        game.updateAllocCount = perf::ReadAllocCounter();

#ifndef NDEBUG
        if (game.updateMs > 2.0f) {
            std::fprintf(stderr, "[PERF WARNING] Update() took %.3f ms (> 2ms budget)\n", game.updateMs);
        }
        if (game.updateAllocCount > 0) {
            std::fprintf(stderr, "[PERF WARNING] %d heap allocation(s) during Update()\n", game.updateAllocCount);
        }
#endif
        ...
        game.renderMs = std::chrono::duration<float, std::milli>(renderEnd - renderStart).count();
        ...
        if (game.screenshotRequested) {
            ...
            std::snprintf(game.screenshotPath, sizeof(game.screenshotPath), "%s", filename);
            game.screenshotNotificationTimer = 3.0f;  // Show notification for 3 seconds
